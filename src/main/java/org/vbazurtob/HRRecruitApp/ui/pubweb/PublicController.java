 ...
 *     <p>Controller for all the public pages
 */
@Controller
...
public class PublicController implements ControllerEndpoints {
  ...
  public String HomepageController(Model model) {
    model.addAttribute("applicantLoginUrl", APPLICANT_LOGIN_URL);
  ...
  public String LoginController(Model model) {
    model.addAttribute("applicantSignupUrl", SIGNUP_NEW_APPLICANT_URL);
  ...
  public String createNewApplicant(
    ...
    if (applicantService.usernameExists(newApplicantForm.getUsername())) {
      results.addError(
          new ObjectError(
              "UsernameAlreadyExists", "The username already exists! Choose another one"));
    }
    ...
    applicantService.createNewApplicantInDB(
        newApplicantForm.getUsername(),
        newApplicantForm.getPassword(),
        newApplicantForm.getEmail());
