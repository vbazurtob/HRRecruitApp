 ...
 *     <p>Controller for all operations related to the applicant cv and personal profile
 */
@Controller
...
public class PersonalProfileController {
  ...
  @Autowired private ApplicantWorkExpService applicantWorkExpService;

  @Autowired private ApplicantWorkExpRepository applicantWorkExpRepository;

  @Autowired private ApplicantSkillService applicantSkillService;

  @Autowired private ApplicantSkillRepository applicantSkillRepository;

  @Autowired private ApplicantAcademicsRepository applicantAcademicRepository;

  @Autowired private CountryService countryService;

  @Autowired private ProficiencyService proficiencyService;

  @Autowired private ApplicantAcademicsService applicantAcademicsService;

  @Autowired private ApplicantAcademicsRepository appAcademicsRepository;

  @Autowired private TypeDegreeService degreeTypeService;
  ...
  public String updatePassword(
    ...
    if (!applicantService.currentApplicantPasswordMatch(
        applicantChangePwdForm.getUsernameChangePwdForm(),
        applicantChangePwdForm.getCurrentPassword())) {
      ObjectError errorCurrentPasswordIsNotValid =
          new ObjectError(
              "CurrentPasswordIsNotValid",
              "Current password for applicant "
                  + applicantChangePwdForm.getUsernameChangePwdForm()
                  + " doesn't match with records");
    ...
    if (results.hasErrors()) {
      ...
      ApplicantWithPassword currentApplicant =
          applicantRepository.findOneByUsername(applicantChangePwdForm.getUsernameChangePwdForm());
    ...
    boolean isPwdChanged =
        applicantService.updatePassword(
            applicantChangePwdForm.getUsernameChangePwdForm(),
            applicantChangePwdForm.getPassword(),
            applicantChangePwdForm.getPasswordConfirmation());
    if (isPwdChanged) {
  ...
  public String showAcademics(Model model, @PathVariable Optional<Integer> page) {
    ...
    // Set Applicant nested object for proper validation, we need to pass in the post at least the
    // applicant username
    ...
    Page<ApplicantAcademic> academicsPageObj =
        applicantAcademicsService.getPaginatedRecords(username, page, RECORDS_PER_PAGE);
  ...
  public String showAcademics(
      @Valid @ModelAttribute("academicsForm") ApplicantAcademic academicsForm,
      BindingResult results,
      RedirectAttributes redirectAttrs,
      Model model) {
    ...
    if (applicantAcademicsService.recordExists(
      ...
      ObjectError errorDuplicateRecord =
          new ObjectError("AcademicsRecordDuplicate", "Records already exists in db!");
    ...
    // Pagination for listing
    Page<ApplicantAcademic> academicsPageObj =
        applicantAcademicsService.getPaginatedRecords(
            username, Optional.of(Integer.valueOf(0)), RECORDS_PER_PAGE);
    long previousPageNum = applicantAcademicsService.getPaginationNumbers(academicsPageObj)[0];
    long nextPageNum = applicantAcademicsService.getPaginationNumbers(academicsPageObj)[1];
  ...
  public String editAcademics(Model model, @PathVariable Long id) {
  ...
  @DeleteMapping(
      value = APPLICANT_ACADEMICS_PAGE + "delete/{id}",
  ...
  public String showWorkExperience(Model model, @PathVariable Optional<Integer> page) {
    ...
    // Pagination for listing
    Page<ApplicantWorkExperience> workExpPageObj =
        applicantWorkExpService.getPaginatedRecords(username, page, RECORDS_PER_PAGE);
  ...
  public String editWorkExp(
      @Valid @ModelAttribute("workexpForm") ApplicantWorkExperience workexpForm,
      BindingResult results,
      RedirectAttributes redirectAttrs,
      Model model) {
    ...
    if (applicantWorkExpService.recordExists(
      ...
      ObjectError errorDuplicateRecord =
          new ObjectError("WorkExpRecordDuplicate", "Records already exists in db!");
      results.addError(errorDuplicateRecord);
    }
    ...
    // Pagination for listing
    Page<ApplicantWorkExperience> workexpPageObj =
        applicantWorkExpService.getPaginatedRecords(
            username, Optional.of(Integer.valueOf(0)), RECORDS_PER_PAGE);
    long previousPageNum = applicantWorkExpService.getPaginationNumbers(workexpPageObj)[0];
    long nextPageNum = applicantWorkExpService.getPaginationNumbers(workexpPageObj)[1];
  ...
  public String editWorkExp(Model model, @PathVariable Long id) {
  ...
  @DeleteMapping(
      value = APPLICANT_WORKEXP_PAGE + "delete/{id}",
      produces = {MediaType.APPLICATION_JSON_VALUE})
  @ResponseBody
  ...
  public String showSkills(Model model, @PathVariable Optional<Integer> page) {
    ...
    // Pagination for listing
    Page<ApplicantSkill> skillPageObj =
        applicantSkillService.getPaginatedRecords(username, page, RECORDS_PER_PAGE);
  ...
  public String showSkills(
      @Valid @ModelAttribute("skillsForm") ApplicantSkill skillForm,
      BindingResult results,
      RedirectAttributes redirectAttrs,
      Model model) {

    // Check if record already exists (DB validation)
    if (applicantSkillService.recordExists(
        skillForm.getApplicant().getUsername(), skillForm.getName(), skillForm.getProficiency())) {
      ObjectError errorDuplicateRecord =
          new ObjectError("SkillsRecordDuplicate", "Records already exists in db!");
      results.addError(errorDuplicateRecord);
    }
    ...
    // Pagination for listing
    Page<ApplicantSkill> skillPageObj =
        applicantSkillService.getPaginatedRecords(
            username, Optional.of(Integer.valueOf(0)), RECORDS_PER_PAGE);
    long previousPageNum = applicantSkillService.getPaginationNumbers(skillPageObj)[0];
    long nextPageNum = applicantSkillService.getPaginationNumbers(skillPageObj)[1];
  ...
  public String editSkills(Model model, @PathVariable Long id) {
  ...
  public String editSkills(
      @Valid @ModelAttribute("skillsForm") ApplicantSkill skillsForm,
      BindingResult results,
      RedirectAttributes redirectAttrs,
      Model model,
      @PathVariable Long id) {
  ...
  @DeleteMapping(
      value = APPLICANT_SKILLS_PAGE + "delete/{id}",
      produces = {MediaType.APPLICATION_JSON_VALUE})
  @ResponseBody
  ...
  public String viewMyCV(Model model) {
    ...
    model.addAttribute(
        "workexpLst",
        applicantWorkExpRepository.findByApplicantUsernameOrderByStartedDescFinishedDesc(username));

    model.addAttribute(
        "educationLst",
        applicantAcademicRepository.findByApplicantUsernameOrderByStartedDescFinishedDesc(
            username));

    model.addAttribute(
        "skillsLst",
        applicantSkillRepository.findByApplicantUsernameOrderByProficiencyDescNameDesc(username));
