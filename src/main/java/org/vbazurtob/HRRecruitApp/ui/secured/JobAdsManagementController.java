 ...
 *     <p>Controller for all operations related to manage job advertisements that will be posted by
 *     an hr department member of the company
 */
...
public class JobAdsManagementController implements ControllerEndpoints {
  ...
  @Autowired private JobApplicantService jobApplicantService;
  ...
  @Autowired private ApplicantAcademicsService applicantAcademicService;
  ...
  @RequestMapping(
      value = {HR_MEMBER_JOBS_MANAGEMENT_PAGE, HR_MEMBER_JOBS_MANAGEMENT_PAGE + "/{page}"})
  ...
  public String saveProfile(
    ...
    if (results.hasErrors()) {
      ...
      ArrayList<JobType> jobTypeList =
          (ArrayList<JobType>) jobTypeRepository.findAllByOrderByDescriptionAsc();
      model.addAttribute(
          "formActionUrl", controllerMapping + HR_MEMBER_JOBS_MANAGEMENT_PAGE + "new");
      model.addAttribute("baseUrl", controllerMapping + HR_MEMBER_JOBS_MANAGEMENT_PAGE);
      model.addAttribute("formTitle", "Create new job");
  ...
  public String editJob(Model model, @PathVariable Long id) {
    ...
    ArrayList<JobType> jobTypeList =
        (ArrayList<JobType>) jobTypeRepository.findAllByOrderByDescriptionAsc();
    model.addAttribute(
        "formActionUrl", controllerMapping + HR_MEMBER_JOBS_MANAGEMENT_PAGE + "edit/" + id);
    model.addAttribute("baseUrl", controllerMapping + HR_MEMBER_JOBS_MANAGEMENT_PAGE);
  ...
  public String editJob(
    ...
    if (results.hasErrors()) {
      ...
      model.addAttribute(
          "formActionUrl",
          controllerMapping + HR_MEMBER_JOBS_MANAGEMENT_PAGE + "edit/" + jobForm.getId());
      model.addAttribute("baseUrl", controllerMapping + HR_MEMBER_JOBS_MANAGEMENT_PAGE);
  ...
  @DeleteMapping(
      value = HR_MEMBER_JOBS_MANAGEMENT_PAGE + "delete/{id}",
  ...
  @PutMapping(
      value = HR_MEMBER_JOBS_MANAGEMENT_PAGE + "close/{id}",
      produces = {MediaType.APPLICATION_JSON_VALUE})
  @ResponseBody
  ...
  @RequestMapping(
      value = {HR_MEMBER_APPLICANTS_JOB + "{index}", HR_MEMBER_APPLICANTS_JOB + "{index}/{page}"})
  public String viewApplicantsForJob(
    ...
    // Pagination for listing
    Page<JobApplicant> applicantsJobObj =
        jobApplicantService.getPaginatedApplicantsFromJob(index, page, RECORDS_PER_PAGE);
    ...
    model.addAttribute(
        "jobObj",
        applicantsJobObj.getContent().size() > 0
            ? applicantsJobObj.getContent().get(0).getJob()
            : null);
    ...
    model.addAttribute(
        "viewCVUrl",
        controllerMapping
            + HR_MEMBER_VIEW_APPLICANT_CV
            + (applicantsJobObj.getContent().size() > 0
                ? applicantsJobObj.getContent().get(0).getApplicant().getUsername()
                : null));
  ...
  public String viewApplicantCV(@PathVariable String username, Model model) {
