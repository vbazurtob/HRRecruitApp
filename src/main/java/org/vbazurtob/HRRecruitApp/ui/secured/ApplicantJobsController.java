 ...
 *     <p>Controller for applicant job related operations
 */
...
public class ApplicantJobsController {
  ...
  private static final int RECORDS_PER_COLUMN = 4;
  ...
  @Autowired private JobSearchFiltersService jobSearchFiltersService;
  ...
  public String searchJobs(
    ...
    model.addAttribute(
        "clearFilterFormUrl", controllerMapping + APPLICANT_JOB_POST_FILTER_JOB_CLEAR);
  ...
  public String viewJobDetails(Model model, @PathVariable Long id) {
    ...
    boolean alreadyApplied =
        jobApplicantRepository.countByApplicantUsernameAndJobId(username, id) > 0;

    if (alreadyApplied) {
      JobApplicant jaDetails =
          jobApplicantRepository.findByApplicantUsernameAndJobId(username, id).get(0);
  ...
  @RequestMapping(
      value = {
        APPLICANT_JOB_APPLICANTS_JOBS_APPLIED,
        APPLICANT_JOB_APPLICANTS_JOBS_APPLIED + "/{page}"
      })
  public String myAppliedJobs(
    ...
    // Pagination for listing
    Page<JobApplicant> jobAppliedPageObj =
        jobApplicantService.getPaginatedRecords(username, page, RECORDS_PER_PAGE);
